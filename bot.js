...
function respond() {
  var request = JSON.parse(this.req.chunks[0]),
      ...
      botRegexTrade = /^\/trades/; botRegex4th = /^\/4th/; botRegexBills = /^\/bills/; botRegexStr = /^\/stream/ 
  ...
  if(request.text && botRegex.test(request.text)) {
    ...
    this.res.writeHead(200);
    postMessage("http://daddyleagues.com/MWO/team/"+request.text.substring(5,8)+"/depthchart");
    this.res.end();
  } 
    ...
    this.res.writeHead(200);
    postMessage("https://www.daddyleagues.com/MWO/rules");
    this.res.end();
  } 
  else if(request.text && botRegexSC.test(request.text)) {
    this.res.writeHead(200);
    postMessage("http://daddyleagues.com/MWO/team/"+request.text.substring(5,8)+"/schedule");
    this.res.end();
  }
    ...
    var rep = req.replace(/ /,"+");
    postMessage("http://daddyleagues.com/MWO/players?name="+rep+"&position=all&team=all");
    this.res.end();
  }  
    ...
    this.res.writeHead(200);
    postMessage("https://www.daddyleagues.com/MWO/schedules");
    this.res.end();
  } 
    ...
    this.res.writeHead(200);
    postMessage("http://daddyleagues.com/MWO/forum/topic/28916");
    this.res.end();
  } 
    ...
    this.res.writeHead(200);
    postMessage("http://daddyleagues.com/MWO/standings");
    this.res.end();
  }
    ...
    this.res.writeHead(200);
    postMessage("https://i.imgur.com/xFYHSMv.png");
    this.res.end(); 
  }
